...
{
    int w = M_SIZE * 36;
    tft.drawRect(x, y, w, M_SIZE * 155, TFT_GREY);
    tft.fillRect(x + 2, y + 19, w - 3, M_SIZE * (155 - 38), TFT_WHITE);
    ...
    tft.fillTriangle(x + 3, y + M_SIZE * 127, x + 3 + 16, y + M_SIZE * 127, x + 3, y + M_SIZE * 127 - 5, TFT_RED);
    tft.fillTriangle(x + 3, y + M_SIZE * 127, x + 3 + 16, y + M_SIZE * 127, x + 3, y + M_SIZE * 127 + 5, TFT_RED);
    M_SIZE *
...
void plotPointer(void)
{
    ...
    for (int i = 0; i < 6; i++)
    {
        ...
        tft.drawRightString(buf, M_SIZE * (i * 40 + 36 - 5), M_SIZE * (187 - 27 + 155 - 18), 2);

        int dx = 3 + M_SIZE * 40 * i;
        if (value[i] < 0)
