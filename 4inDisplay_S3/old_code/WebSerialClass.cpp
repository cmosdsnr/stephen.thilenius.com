#include "WebSerialClass.h"
#include "Serial.h"
#include "Clock.h"
#include "HostName.h"

/**
 * @brief Starts the WebSerial service.
 *
 * @param server Pointer to AsyncWebServer
 * @param url URL path for the service (default /)
...
size_t WebSerialClass::write(const uint8_t *buffer, size_t size, int32_t id)
{
    if (_ws != NULL)
    {
        _ws->text(id, (const char *)buffer, size);
        return size;
    }
    else
        return 0;
