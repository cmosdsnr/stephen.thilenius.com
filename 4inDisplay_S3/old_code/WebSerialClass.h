...
/**
 * @brief Web-based Serial Monitor implementation.
 *
 * Provides a Serial-like interface over WebSocket to a browser.
 */
class WebSerialClass : public Print
{
public:
    /**
     * @brief Starts the WebSerial service on the web server.
     */
    void begin(AsyncWebServer *server, const char *url = "/", const String &username = "", const String &password = "");
    ...
    size_t write(const uint8_t *buffer, size_t size, int32_t id);
