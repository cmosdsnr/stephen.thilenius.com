...
{
    if (fullPath == nullptr || dest == nullptr)
        return;
    ...
    if (lastSlash)
    {
        ...
        if (dirLen >= FTP_CWD_SIZE)
            dirLen = FTP_CWD_SIZE - 1;
    ...
    if (dot)
    {
        ...
        for (size_t i = 0; i < nlen && bi < 8; ++i)
        {
            ...
            if (isalnum((unsigned char)c))
                base[bi++] = toupper((unsigned char)c);
            else
                base[bi++] = '_';
        }
        base[bi] = '\0';
        ...
        for (size_t i = 0; i < elen && ei < 3; ++i)
        {
            char c = dot[1 + i];
            if (isalnum((unsigned char)c))
                ext[ei++] = toupper((unsigned char)c);
            else
                ext[ei++] = '_';
        }
        ext[ei] = '\0';
...
bool FtpServer::openDir(FTP_DIR *pdir)
{
    ...
    if (dir)
    {
        DEBUG_PRINTLN(F("FTP DBG: Open dir success"));
        if (dir.isDirectory())
        {
            DEBUG_PRINTLN(F("FTP DBG: Is directory"));
        }
        else
        {
            DEBUG_PRINTLN(F("FTP DBG: NOT directory"));
        }
    }
    else
    {
        DEBUG_PRINTLN(F("FTP DBG: Open dir FAILED"));
    }
    openD = true;
    if (!openD)
    {
