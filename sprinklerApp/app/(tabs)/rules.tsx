/**
 * @module app/(tabs)/rules
 * @description **Rules tab** — manage the repeating watering schedule via an
 * abstract rules interface.
 *
 * A **rule** defines a start time, per-channel durations, and the set of
 * cycle days it applies to. The tab lists all current rules as color-coded
 * cards. Tapping a card opens {@link RuleModal} for editing; the "+ New Rule"
 * button opens the same modal in create mode.
 *
 * ### RuleModal
 * The inline `RuleModal` component handles all rule creation and editing. It
 * provides:
 * - Day selectors for both weeks of the 14-day cycle with "All" shortcuts.
 * - ±5 / ±15 / ±60 minute start-time adjusters.
 * - Per-channel duration inputs (minutes).
 * - Save (disabled until at least one day and one non-zero duration are set),
...
/** A rule that may or may not have an id yet (create vs. edit). */
type RuleInput = Omit<Rule, 'id'> & { id?: number };

const WEEK1 = Array.from({ length: 7 }, (_, i) => i);
const WEEK2 = Array.from({ length: 7 }, (_, i) => i + 7);

/**
 * Return an empty rule template with no days selected and zero durations.
 *
 * @returns {RuleInput}
 */
...
/**
 * Format minutes-from-midnight as a 12-hour clock string (e.g. `6:00 AM`).
 *
...
/**
 * Compute the end time of a rule in minutes-from-midnight by summing all
 * channel durations.
 *
 * @param {RuleInput} rule
 * @returns {number}
 */
function endTime(rule: RuleInput): number {
...
/**
 * Read-only row showing which days in one week have the rule enabled.
 *
 * @param {WeekDayRowProps} props
 */
function WeekDayRow({ label, week, rule, rowStyle }: WeekDayRowProps) {
...
/**
 * Full-screen modal for creating or editing a single rule.
 *
 * @param {RuleModalProps} props
 * @returns {React.ReactElement|null}
 */
function RuleModal({ rule, onSave, onDelete, onCancel }: RuleModalProps) {
    const [editing, setEditing] = useState<RuleInput>(() => rule ?? emptyRule());
    ...
    const setDuration = (ch: number, val: string) => {
...
/**
 * Rules tab screen component.
 *
 * @returns {React.ReactElement}
 */
export default function RulesTab() {
    const { rules, addRule, saveRule, deleteRule } = useSprinkler();
    const [modalRule, setModalRule] = useState<RuleInput | null>(null);

    function openNew() { setModalRule(emptyRule()); }
    function openEdit(rule: Rule) { setModalRule(rule); }
    function handleSave(rule: RuleInput) {
        if (rule.id != null) saveRule(rule as Rule);
        else addRule(rule);
        setModalRule(null);
    }
    function handleDelete(id: number) { deleteRule(id); setModalRule(null); }
