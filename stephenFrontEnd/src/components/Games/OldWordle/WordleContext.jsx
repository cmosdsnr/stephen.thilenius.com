...
export function WordleProvider({ children }) {
    ...
    useEffect(() => {
        const loadWords = async () => {
            var w = [];
            var r = await fetch(level0words);
            var t = await r.text();
            t.split("\n").forEach(line => line.split(" ").forEach(word => w.push(word.slice(0, 5).toUpperCase())));
            setLevel0Words(w);
            setWordleWords(w);
            var x = [];
            r = await fetch(level1words);
            t = await r.text();
            t.split("\n").forEach(line => line.split(" ").forEach(word => x.push(word.slice(0, 5).toUpperCase())));
            setLevel1Words(x);
            var y = [];
            r = await fetch(level2words);
            t = await r.text();
            t.split("\n").forEach(line => line.split(" ").forEach(word => y.push(word.slice(0, 5).toUpperCase())));
            y = [...y, ...x, ...w]

            // sort y alphabetically
            y.sort();

            // remove invalid words from pb
            const record = await pb.collection('miscellaneous').getOne("000invalidwords");
            if (record && record.data && record.data.words) {
                console.log("Loaded invalid words from pb with length: ", record.data.words.length);
                y = y.filter(word => !record.data.words.includes(word));
            }
            else
                console.log("No invalid words record found in pb.");
            setWords(y);
        }
        loadWords()
        ...
        const fixWords = async () => {
            ...
            const text = await r.text()
            text.split("\n").forEach(line => line.split(" ").forEach(word => w.push(word.slice(0, 5).toUpperCase())))
            // search for words that are in wordListFile but not in longWordListFile
            const r2 = await fetch(longWordListFile)
            const text2 = await r2.text()
            text2.split("\n").forEach(line => line.split(" ").forEach(word => x.push(word.slice(0, 5).toUpperCase())))
            const missingWords = w.filter(word => !x.includes(word))
            if (missingWords.length > 0) {
                console.log("Missing words: ", missingWords)
            } else {
                console.log("All words are present in the long word list.")
            }
        }
        // fixWords()
    }, [])
